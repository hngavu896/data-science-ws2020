...
```{r}
library(readr)
titanic <- read_delim("~/UNI/WS20 - R/praxis/data-science-ws2020/titanic.csv", 
...
```{r}
model_svm <- svm(survived ~ ., data = tdata)
summary(model_svm)
pred1 <- predict(model_svm, vdata, probability = FALSE)
```
...
```{r}
pROC_obj <- roc(test.results_svm$survived, test.results_svm$pred,
            smoothed = TRUE,
