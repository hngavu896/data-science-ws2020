...
wss <- (nrow(book_matrix)-1)*sum(apply(book_matrix,2,var))
  for (i in 2:10) wss[i] <- sum(kmeans(book_matrix, centers = i)$withinss)
...
plot(book.hc)
class <- cutree(book.hc, k = 3)
rect.hclust(book.hc, k=3, border = "blue")
```
...
```{r}
set.seed(1234)
k.means.fit <- kmeans(book_matrix, 3)
```
...
Leider bekomme ich immer Fehlermeldung, wenn ich Plot für K-Means Clustering erstelle
